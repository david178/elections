...
Partial Public Class EVTurnout_SP
    ...
    Private Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        ElseIf e.Row.RowType = DataControlRowType.Footer Then
            e.Row.Cells(0).Text = "Total Acumulado"
    ...
    Private Sub GetEVTurnoutGridResults()
        ...
        Try
            ...
            sSQL = "select upper(replace(replace(upper(substr(elecname,6,length(elecname)-5)),'PRIMARY ELECTION','Elección Primaria'),'GENERAL ELECTION','Elección General'))||' '||substr(elecname,1,4) from cl_web_election where default_elec='Y'"
            'sSQL = "select upper(replace(replace(upper(substr(elecname,6,length(elecname)-5)),'PRIMARY ELECTION','ELECCIÓN PRIMARIA MUNICIPAL'),'MUNICIPAL GENERAL','ELECCIÓN GENERAL MUNICIPAL'))||' '||substr(elecname,1,4) from cl_web_election where default_elec='Y'"

            Dim cmd As New OracleCommand(sSQL)
            cmd.Connection = cn
            cn.Open()
            sElectionName = cmd.ExecuteScalar
            'cmd.Connection.Close()
            cmd.Dispose()

            lblElection.Text = sElectionName & "<br>Participación Electoral en la Votación Temprana"
            ...
            If iSessions > 0 Then
                ...
                'Dim sUrl As String = "EV_Schedules/" & sElectionCode & "_EV_SCHED_SP.pdf"
                    ...
                    If rdr.HasRows Then
                        ...
                        For x = 1 To iSites
                            ...
                            If rdr.HasRows Then
                                ...
                                '    lblTextNLV.Text = Replace(Replace(rdr.GetValue(0).ToString, "Mobile Team", "Equipo Móvil"), "Trailer", "Tráiler") & " (" & rdr.GetValue(1).ToString() & ")"
                                'Else
                                '    lblTextNLV.Text = Replace(Replace(rdr.GetValue(0).ToString, "Mobile Team", "Equipo Móvil"), "Trailer", "Tráiler")
                                'End If
                ...
                If iSessions < 8 Then
                    ...
                    lblWeek1.Text = "Participación Diaria"
                ElseIf iSessions = 14 Then
                    sSQL = "select dtSession, rownum from (select dtSession from (select distinct to_char(ev_session, 'mm/dd') " & _
                        "dtSession from cl_ev_turnout where election_code='" & sElectionCode & "' order by 1) order by 1) " & _
                        "where rownum <= " & (iSessions - 7) & " order by 1"
                    lblWeek1.Text = "Participación Diaria - Semana 1"
                Else
                ...
                If rdr.HasRows Then
                    ...
                    For x = 1 To iSites
                        ...
                        If rdr.HasRows Then
                            ...
                            rdr.Read()
                            'If rdr.GetValue(1).ToString <> "CC" And UCase(Left(rdr.GetValue(0).ToString, 11)) = "MOBILE TEAM" Then
                            '    lblText1.Text = Replace(Replace(rdr.GetValue(0).ToString, "Mobile Team", "Equipo Móvil"), "Trailer", "Tráiler") & " (" & rdr.GetValue(1).ToString() & ")"
                            'Else
                            '    lblText1.Text = Replace(Replace(rdr.GetValue(0).ToString, "Mobile Team", "Equipo Móvil"), "Trailer", "Tráiler")
                            'End If
                    ...
                    If rdr.HasRows Then
                        ...
                        For x = 1 To iSites
                            ...
                            If rdr.HasRows Then
                                ...
                                If rdr.GetValue(1).ToString <> "CC" And UCase(Left(rdr.GetValue(0).ToString, 11)) = "MOBILE TEAM" Then
                                    lblText2.Text = Replace(Replace(rdr.GetValue(0).ToString, "Mobile Team", "Equipo Móvil"), "Trailer", "Tráiler") & " (" & rdr.GetValue(1).ToString() & ")"
                                Else
                                    lblText2.Text = Replace(Replace(rdr.GetValue(0).ToString, " Mobile Team", ", Equipo Móvil"), "Trailer", "Tráiler")
                                End If
