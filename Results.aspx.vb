...
Partial Public Class Results
    ...
    Private Sub GetEVTurnoutGridResults()
        ...
        Try
            ...
            If iSessions > 0 Then
                ...
                If iSites > 1 Then
                    gvSiteTurnout.Visible = True
                Else
                    gvSiteTurnout.Visible = False
                End If
    ...
    Private Sub btnGenerateVH_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGenerateVH.Click
        If sElectionCode <> "" Then
            ...
            Dim ad As New OracleDataAdapter("select '""'||idnumber||'"",""'||votername||'"",""'||voting_method||'"",""'||precinct||'"",""'||ballot_party||'"",""'||voter_party||'"",""'||to_char(date_voted,'mm/dd/yyyy')||'"",""'||election_code||'""' vh_rec " & _
                "from clark.cl_vote_history where election_code='" & sElectionCode & "' order by precinct, voter_party", cn)

            ad.Fill(ds)

            Dim str As New StringBuilder()

            str.Append("""" & "idnumber" & """" & "," & """" & "votername" & """" & "," & """" & "voting_method" & """" & "," & """" & "precinct" & """" & "," & """" & "ballot_party" & """" & "," & """" & "voter_party" & """" & "," & """" & "date_voted" & """" & "," & """" & "election_code" & """")
            str.AppendLine()
    ...
    Private Sub btnPrintVer_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrintVer.Click
        Dim popupScript As String = "<script language='javascript'>" & _
            "window.open('Results_pv.aspx?c=" & sElectionCode & "&e=" & ddlElection.SelectedItem.Text & "', '', " & _
            "'width=600, height=600, menubar=yes, resizable=yes, scrollbars=yes')" & _
            "</script>"
        ClientScript.RegisterStartupScript(GetType(String), "PopupScript_WL", popupScript)
    End Sub
    ...
    Private Sub gvSiteTurnout_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvSiteTurnout.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
